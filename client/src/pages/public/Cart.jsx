      ...
      {STEPS.map((label, index) => (
        <React.Fragment key={label}>
          <div className="flex items-center gap-2">
            <div
              className={`flex h-8 w-8 items-center justify-center rounded-xl text-xs font-bold ${index <= step ? "bg-blue-700 text-white" : "bg-slate-200 text-slate-500"}`}>
              {index < step ? "OK" : index + 1}
            </div>
            <span
              className={`hidden text-sm font-semibold sm:block ${index === step ? "text-blue-700" : index < step ? "text-cyan-700" : "text-slate-400"}`}>
              {label}
            </span>
          </div>
          {index < STEPS.length - 1 && (
            <div
              className={`h-0.5 w-8 rounded-full ${index < step ? "bg-cyan-400" : "bg-slate-200"}`}
            />
          )}
        </React.Fragment>
      ))}
  ...
  return (
    <aside className="space-y-4">
      <div className="card">
        ...
        <div className="mt-4 space-y-2 border-t border-slate-100 pt-4 text-sm">
          ...
          <div className="flex justify-between">
            ...
            <span>{deliveryFee ? money(deliveryFee) : "Free / pending"}</span>
          </div>
          {depositTotal > 0 && (
      ...
      <div className="rounded-2xl border border-cyan-100 bg-cyan-50 p-4 text-sm text-cyan-900">
        UPI and QR payments stay pending until an admin verifies the screenshot
        and UTR.
      </div>
    </aside>
  );
...
export default function Cart() {
  ...
  const [checkServiceArea, { isLoading: checkingArea }] =
    useCheckServiceAreaMutation();
  ...
  const depositTotal = useMemo(
    () =>
      jarItems.reduce((sum, item) => {
        const option = jarOptions[item.product] || "no_jar";
        return ["no_jar", "return_later"].includes(option)
          ? sum + jarDepositAmount * item.quantity
          : sum;
      }, 0),
    [jarItems, jarOptions, jarDepositAmount],
  );
  const deliveryFee = Number(delivery?.deliveryFee || 0);
  const total = Math.max(
    0,
    subtotal + depositTotal + deliveryFee - couponDiscount,
  );
  ...
  const slotLabel = slot
    ? `${dates[selectedDate]?.label}, ${slot.name} (${slot.startTime}-${slot.endTime})`
    : "";
  ...
  const checkPincode = async (address) => {
    ...
    try {
      ...
      if (!result.isServiceable) {
        setDelivery(null);
        toast.error(
          result.message || "Delivery is not available for this pincode",
        );
        return null;
      }
      setDelivery(result);
  ...
  const goToSlot = async () => {
    if (!ensureLoggedIn()) return;
    const hasAddress =
      selectedAddress?.fullAddress || manualAddress.fullAddress.trim();
    ...
    if (Number(currentDelivery.minimumOrder || 0) > subtotal) {
      return toast.error(
        `Minimum order for this area is ${money(currentDelivery.minimumOrder)}`,
      );
    }
    setStep(2);
  };
  ...
  const goToPayment = () => {
    if (!activeSlots.length)
      return toast.error("No active delivery slot is configured");
  ...
  const handleFile = (event) => {
    ...
    reader.onload = (loadEvent) =>
      setScreenshotPreview(loadEvent.target.result);
  ...
  const placeOrder = async () => {
    ...
    if (!delivery?.isServiceable)
      return toast.error("Check delivery availability first");
    if (Number(delivery.minimumOrder || 0) > subtotal)
      return toast.error(
        `Minimum order for this area is ${money(delivery.minimumOrder)}`,
      );
    if (!selectedSlot) return toast.error("Select a delivery slot");
    ...
    try {
      const order = await createOrder({
        ...
        paymentMethod,
        orderNotes: [notes, slotLabel ? `Slot: ${slotLabel}` : ""]
          .filter(Boolean)
          .join(" | "),
      ...
      if (paymentMethod === "upi") {
        ...
        try {
          ...
          toast.error(
            proofErr?.data?.message ||
              "Order placed, but proof upload failed. Submit it from My Payments.",
          );
        }
      }
    ...
    return (
      <div className="grid min-h-[60vh] place-items-center bg-slate-50 px-4">
        <div className="max-w-sm text-center">
          <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-blue-50 text-xl font-black text-blue-700">
            RO
          </div>
          ...
          <p className="mt-2 text-sm text-slate-500">
            Add RO jars, bottles, or cartons before checkout.
          </p>
          ...
          <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="text-sm text-slate-500">
                Address, slot, jar deposit, and manual payment verification.
              </p>
            </div>
